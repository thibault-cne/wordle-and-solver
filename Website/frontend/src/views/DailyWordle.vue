...
<template>
  <div v-bind:class="{ active: isActive }">
      ...
      <div class="row">
        <div class="col-8 col-s-10 center">
          ...
          <div class="game-message-win" v-if="this.retourapi === 'win'">
            Bravo ! <br />
            Vous avez gagné, revenez demain<br />
            ou changez de mode de jeu.
          </div>
          <div
          ...
          >
            Dommage ! <br />
            Vous avez perdu, revenez demain <br />
            ou changez de mode de jeu. <br />
    ...
    <audio autoplay v-if="this.winCondition">
      <source src="../../public/Audio/zizou_masterclass_la_bise.mp3" />
    </audio>
  </div>
</template>
...
export default {
  ...
  async created() {
    ...
    for (let i = this.ligne; i < this.config.nbTries; i++) {
      const arr = [];
      for (let j = 0; j < this.config.wordLength; j++) {
  ...
  methods: {
    async getNewWord() {
      await getRequest("/game-api/daily/create", "json").then((response) => {
        if (response.data.hasPendingGame) {
          this.setUpOldGame(response.data.pendingGame);
        }
        this.config.wordLength = response.data.wordLength;
      });
    },
    ...
    async testMethod() {
      ...
      await postRequest(data, "/game-api/daily/testWord", "json").then(
        (response) => {
    ...
    async setstatuslettre() {
      ...
      for (let i = 0; i < nbLetter; i++) {
        let className = this.getClassName(this.reponse[i]);
